#write by Sun Haiayng at 2024.08.29 [双端模式]
#1、搭环境
{
  rm(list=ls())
...
#2、给索引
{
  ...
  inxD = "/Reference/aaSHY/zOther/TEconsensus/INDEX/MM/STARv279a"
...
#3、跑命令
{
  ...
  cmd_01 <- paste0("trim_galore --phred33 --fastqc --illumina --paired ",
                   "--clip_R1 10 --three_prime_clip_R1 2 ",
                   "--clip_R2 10 --three_prime_clip_R2 2 ",
                   "-j 2 -o ",path,"trim ",lay1," ",lay2,"\n")
...
################################################################################
{
  bw01 <- "/ChIP_seq_2/aaSHY/ghyuu/ripSEQ/Aef/pairEND/dumpROOM/IP-5.bw"
  bw02 <- "/ChIP_seq_2/aaSHY/ghyuu/ripSEQ/Aef/pairEND/dumpROOM/Input-5.bw"
  bw03 <- "/ChIP_seq_2/aaSHY/ghyuu/ripSEQ/ff23k/star/pairEND/bw/bcv/Input.bw"#没有调整peFragmentsize的参数
...
#6、TE Binding situation [RIP-seq Specific]
{
  ...
  #Expected
  mmu1 <- read.table(file = "/path/rnaSEQ/count/Aef.cntTable", sep = "\t", header = T, stringsAsFactors = F, row.names = 1)
