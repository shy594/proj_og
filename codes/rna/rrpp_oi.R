...
#02、给索引
{
  path = "/ChIP_seq_2/aaSHY/rrpp_oi/rnaSeq/specificS/"
...
#03、跑命令
{
  ...
  cmd_04 <- paste0("STAR --runThreadN 4 --readFilesCommand zcat ",
  ...
  cmd_05 <- paste0("mv ",path,"bam/",prex,"Aligned.sortedByCoord.out.bam ",path,"bam/",prex,".bam","\n")
  cmd_06 <- paste0("samtools index ",path,"bam/",prex,".bam","\n")
  cmd_07 <- paste0("bamCoverage --normalizeUsing CPM -p 10 --minMappingQuality 1 ",
  ...
  cmd_08 <- paste0("TEtranscripts -t ",
                   ...
                   "--project rrpp_oi --outdir ",paste0(path,"count"),"\n")
  ...
  cmd_11 <- paste0("stringtie --merge -o ",
                   path,"stringtie/rrpp_oi.merge.gtf -G ",inx3," ",
                   paste(paste0(path,"stringtie/",prex,".gtf"), collapse = " "),"\n")
  cmd_12 <- paste0("cat ", paste0(path,"stringtie/rrpp_oi.merge.gtf "), 
                   "|sed 's/gene_name/gene_abcd/g; s/\\tgene_id/\\tgene_name/g' >",
                   paste0(path,"stringtie/rrpp_oi.merge.R.gtf"),"\n")
  cmd_13 <- paste0("TEtranscripts ",
                   "-t ",paste(paste0(path,"bam/",prex[1:2],".bam"),collapse = " ")," ",
                   "-c ",paste(paste0(path,"bam/",prex[3:4],".bam"),collapse = " ")," ",
                   "--GTF ",path,"stringtie/rrpp_oi.merge.R.gtf ",
                   "--TE ", inx4," --sortByPos --mode multi --project rrpp_oi ",
  ...
  #
  #two-pass mode是否有影响
  cmd_14 <- paste0("STAR --runThreadN 4 --readFilesCommand zcat ",
                   "--outMultimapperOrder Random --outSAMtype BAM SortedByCoordinate ",
  ...
  cmd_17 <- paste0("TEtranscripts -t ",
                   ...
                   " --GTF ",inx3," --TE ",inx4," --sortByPos --mode multi ",
                   "--project rrpp_oi --outdir ",paste0(path,"dumpROOM"),"\n")
  ...
  print(paste0("nohup bash ",shell, " >",
               path,"Log/",basename(shell),".log"," 2>&1 &"))
}
################################################################################
#0A、带和不带2-pass mode, 对差异倍数的影响
{
  tmp1 <- "/ChIP_seq_2/aaSHY/rrpp_oi/rnaSeq/specificSNot/count/rrpp_oi_sigdiff_gene_TE.txt"
  tmp1 <- read.table(tmp1)
  tmp2 <- "/ChIP_seq_2/aaSHY/rrpp_oi/rnaSeq/specificSNot/dumpROOM/rrpp_oi_sigdiff_gene_TE.txt"
...
#04、DESeq2：Gene、TE、TE sites
{
  ...
  shy_diff(mark = "rrpp_oiKO", 
           geneFile = paste0(path, "count/rrpp_oi.cntTable"),
           siteFile = paste0(path, "count/rrpp_oi.site.cntTable"),
           Class = factor(c("KO","KO","WT","WT")), VS = c("Class","KO","WT"))
}
################################################################################
#05、CPM Excel
{
  cnt1 <- read.table(paste0(path,"count/rrpp_oi.cntTable"), header = T, sep = "\t", row.names = 1)
  ...
  openxlsx::write.xlsx(x = tmp1, file = paste0(path,"count/rrpp_oiKO.cpm.onlyTE.xlsx"))
  openxlsx::write.xlsx(x = tmp2, file = paste0(path,"count/rrpp_oiKO.cpm.onlyGene.xlsx"))
  openxlsx::write.xlsx(x = tmp3, file = paste0(path,"count/rrpp_oiKO.cpm.allGeneTE.xlsx"))
}
################################################################################
#06、Volcano for Genes
{
  ...
  shy_volcano_gene(resFile  = paste0(path, "DESeq2/rrpp_oiKO.geneOnly.csv"),
                   saveFile = paste0(path, "PLOT/rrpp_oiKO.geneOnly.volcano.pdf"))
}
################################################################################
#07、Volcano for TEs
{
  ...
  shy_volcano_te(resFile  = paste0(path, "DESeq2/rrpp_oiKO.te.GeneBackGround.csv"),
                 saveFile = paste0(path, "PLOT/rrpp_oiKO.te.GeneBackGround.volcano.pdf"))
  shy_volcano_te(resFile  = paste0(path, "DESeq2/rrpp_oiKO.teOnly.csv"),
                 saveFile = paste0(path, "PLOT/rrpp_oiKO.teOnly.volcano.pdf"))
}
################################################################################
#08、MA
{
  ...
  shy_teMA(difffile = paste0(path,"DESeq2/rrpp_oiKO.teOnly.csv"),
           savefile = paste0(path,"PLOT/rrpp_oiKO.teOnly.MA.pdf"))
}
################################################################################
#09、MA plot TE sites
{
  ...
  shy_repSite(resdFile = paste0(path,"DESeq2/rrpp_oiKO.te.site.teOnly.csv"),
              saveFile = paste0(path,"PLOT/rrpp_oiKO.te.site.teOnly.mervl.pdf"))
}
################################################################################
#10、PCA
{
  #----------------------------------PCA-batch----------------------------------
  cnt1 <- read.table(paste0(path, "count/rrpp_oi.cntTable"),
...
#11、pheatmap
{
  ...
  shy_pheatmap(mark = "heat", readsFile = paste0(path, "count/rrpp_oi.cntTable"),
               gene_resFile = paste0(path, "DESeq2/rrpp_oiKO.geneOnly.csv"),
               tete_resFile = paste0(path, "DESeq2/rrpp_oiKO.teOnly.csv"),
...
#12、Up TEs loci number [TElocal]
{
  shy_lociNum <- function(resdFile, saveFile){
    ...
    p_1 <- ggplot(forP, aes(Var1, Freq, fill = clas)) + 
      ...
      ylab("Number of Loci") + 
      xlab(NULL) +
      labs(title = expression(deRegulated~TEs~"in"~italic(rrpp_oi)^{"+/+"}~ESCs)) +
  ...
  shy_lociNum(resdFile = paste0(path, "DESeq2/rrpp_oiKO.te.site.teOnly.upup.csv"),
              saveFile = paste0(path, "PLOT/TElociNum/rrpp_oi.lociNum.teOnly.upup.pdf"))
  shy_lociNum(resdFile = paste0(path, "DESeq2/rrpp_oiKO.te.site.teOnly.down.csv"),
              saveFile = paste0(path, "PLOT/TElociNum/rrpp_oi.lociNum.teOnly.down.pdf"))
}
################################################################################
#13、MERVL fusion genes expression
{
  shy_fusion_stringTie <- function(countFile, Class, gtfFile, whichTE, contrast){
    ...
    ggsave(plot = p_02, 
           ...
           filename = paste0(path, "PLOT/chimeric/rrpp_oi.chimeric.",whichTE,".pdf"))
  }
  shy_fusion_stringTie(countFile = paste0(path, "stringtie/count/rrpp_oi.cntTable"), 
                       Class = c("KO","KO","WT","WT"),
                       contrast = c("Class","KO","WT"),
                       gtfFile = paste0(path, "stringtie/rrpp_oi.merge.R.gtf"),
                       whichTE = "MT2_Mm")#MT2_Mm#MERVL-int)
}
################################################################################
#14、enrich KEGG
{
  resFile <- "/ChIP_seq_2/aaSHY/rrpp_oi/rnaSeq/DESeq2/rrpp_oiKO.geneOnly.csv"
  resdata <- as.data.frame(read.csv(resFile, header = T, stringsAsFactors = F))
  ...
  ggsave(plot = plot,
         ...
         filename = paste0(path,"PLOT/enrich/","rrpp_oi.kegg.pdf"))
}
################################################################################
#15、enrich GO BP, GO MF
{
  ...
  ggsave(plot = plot,
         units = "cm", width = 22, height = 15,
         filename = paste0(path,"PLOT/enrich/","rrpp_oi.gomf.pdf"))
}
################################################################################
#16、GSEA
{
  shy_gsea <- function(resFile, savePath){
    ...
    af_gsea <- suppressWarnings(clusterProfiler::GSEA(geneList = ae, 
                                                      maxGSSize = 1000, TERM2GENE = ab, pvalueCutoff = 1))
    rm(aa,ab)
  ...
  shy_gsea(resFile = paste0(path, "DESeq2/rrpp_oiKO.geneOnly.csv"), 
           savePath = paste0(path, "PLOT/enrichGSEA/"))
}
################################################################################
#17、python GSEA
{
  temp <- read.csv(paste0(path, "DESeq2/rrpp_oiKO.geneOnly.csv"), header = T)
  temp <- temp[, c("X", "log2FoldChange")]
  temp <- temp[order(temp$log2FoldChange, decreasing = T),]
  write.table(x = temp, file = paste0(path, "DESeq2/rrpp_oi.res.rnk"), quote = F, sep = "\t", col.names = F, row.names = F)
  cmd_12 <- paste0("gseapy prerank -g ","/Reference/aaSHY/GSEAgmt/TwoGenes.gmt -r ",
                   paste0(path, "DESeq2/rrpp_oi.res.rnk"), " -p 10 -o ", 
                   paste0(path, "PLOT/enrichGSEA/"))#conda activate base gseapy 0.10.8
}
################################################################################
#18、UCSC Track
#https://genome.ucsc.edu/s/hysun088/rrpp_oiKO
{
...
#保留了参数: #"bamCompare -p 10 --scaleFactorsMethod SES --sampleLength 100000 "
{
  ...
  bw03 <- list.files(path = "/ChIP_seq_2/aaSHY/Prmt1/ask/MERVL/mt2Activation/bamCompare", full.names = T)
  cmd_01 <- paste0("multiBigwigSummary bins ", 
                   "-p 20 -b ",paste0(c(bw01,bw02,bw03), collapse = " "),
                   " --smartLabels",
                   " -o ",paste0(path,"PLOT/Correlation/rrpp_oi.cor.npz","\n"))
  cmd_02 <- paste0("plotCorrelation -in ",path,
                   "PLOT/Correlation/rrpp_oi.cor.npz ",
                   "-c pearson -p heatmap --plotFileFormat pdf --removeOutliers ",
                   "--colorMap bwr -o ", path,"PLOT/Correlation/rrpp_oi.cor.pdf ",
                   "--outFileCorMatrix ",path,"PLOT/Correlation/rrpp_oi.cor.index.txt","\n")
...
#20、哪些是滋养层基因
{
  ...
  #吕老师说有些不是滋养层基因
  #
  data <- read.table(paste0(path,"count/rrpp_oi.cntTable"), 
...
################################################################################
#21、对rrpp_oi KO后激活的2细胞基因排个序
{
  gata <- read.csv(file = paste0(path,"DESeq2/rrpp_oiKO.geneOnly.csv"),
  ...
  openxlsx::write.xlsx(x = wr01, 
                       file = paste0(path,"DESeq2/rrpp_oiKO.geneOnly.2C.gene.xlsx"), asTable = T)
}
################################################################################
#22、可变剪接
{
  bam1 <- list.files(paste0(path,"bam"), pattern = "KO.*bam$",full.names = T)
...
################################################################################
#23、可变剪接 rmats2sashimiplot
{
  shell <- paste0(path,"src/run_d.sh")
  cat("#!/bin/bash\n", file = shell)
  bam1 <- paste0(list.files(paste0(path,"bam"), pattern = "KO.*bam$", full.names = T), collapse = ",")
  bam2 <- paste0(list.files(paste0(path,"bam"), pattern = "WT.*bam$", full.names = T), collapse = ",")
  for (i in c("A3SS","A5SS","MXE","RI","SE")) {
    if (dir.exists(paths = paste0(path,"rmats2sashimiplot/",i))==F) {
      dir.create(path = paste0(path,"rmats2sashimiplot/",i), recursive = T)
    }
    cmd_01 <- paste0("rmats2sashimiplot -o ",path,
                     "rmats2sashimiplot/",i," --event-type ",i," -e ",path,
                     "rMATS/",i,".MATS.JC.txt --b1 ",bam1," --b2 ",bam2, " --l1 KO --l2 WT ","\n")
    for (m in cmd_01){cat(m, file = shell, append = T)}
  }
  print(paste0("nohup bash ",shell, " >",paste0(path,"Log/"),basename(shell),".log", " 2>&1 &"))
}
#special
{
  bam1 <- paste0(list.files(paste0(path,"bam"), pattern = "KO.*bam$", full.names = T), collapse = ",")
  bam2 <- paste0(list.files(paste0(path,"bam"), pattern = "WT.*bam$", full.names = T), collapse = ",")
  dir.create(path = paste0(path,"rmats2sashimiplot/aaSpecial"), recursive = T)
  cmd_01 <- paste0("rmats2sashimiplot -o ",path,
                   "rmats2sashimiplot/aaSpecial --b1 ",bam1," --b2 ",bam2, " -c ",
                   "1:-:153475758:153487608:",inx5," --l1 KO --l2 WT", "\n")
  print(cmd_01)
}
################################################################################
#
...
#
################################################################################
#01、从bam中拆分出﹢、-链, 制成UCSC track
#结合rrpp_oi文章里的参数描述，判断其链特异建库方式为dUTP（read2的方向即转录本方向）
#samtools view -f 16 -f 128：Read2 且比对到负链
#samtools view -F 16 -f 128：Read2 且比对到非负链（正链）
#bamCoverage转成bw, 参数如下
#--normalizeUsing CPM --minMappingQuality 1 --samFlagExclude 256 -of bigwig -bs 20"
#制成UCSC track：https://genome.ucsc.edu/s/hysun088/rrpp_oiStrand
#
#
#
#
#02、TEtranscripts、STAR、featureCounts、HTseq做定量
{
  #TEtranscripts的定量结果里抽取基因查看, 发现很多是和track 信号对不上
  shell <- paste0(path,"src/run_shy_cnt_a.sh")
  cat("#!/bin/bash\n", file = shell)
  bams <- list.files(paste0(path,"strand/bam"), pattern = "bam$", full.names = T)
  cmd_01 <- paste0("TEtranscripts -t ",paste0(bams[c(2,4,6,8)], collapse = " ")," ",
                   "-c ",paste0(bams[c(1,3,5,7)], collapse = " ")," ",
                   "--stranded reverse --GTF ",inx3," --TE ",inx4," ",
                   "--sortByPos --mode multi ",
                   "--project rrpp_oi --outdir ",path,"strand/count")
  for (i in cmd_01) {cat(i, file = shell, append = T)}
  print(paste0("nohup bash ",shell, " >",
               path,"Log/",basename(shell),".log"," 2>&1 &"))
  #使用HTseq试试
  shell <- paste0(path,"src/run_shy_cnt_b.sh")
  cat("#!/bin/bash\n", file = shell)
  bams <- list.files(paste0(path,"strand/bam"), pattern = "bam$", full.names = T)
  outs <- gsub(".sort.bam","",basename(bams))
  cmd_02 <- paste0("htseq-count -i gene_name -n 4 -s no -r pos ",
                   "-a 10 ",bams," ",inx3," >",path,"strand/count/HTseq/",outs,
                   "-s-no",".txt","\n")
  cmd_03 <- paste0("htseq-count -i gene_name -n 4 -s yes -r pos ",
                   "-a 10 ",bams," ",inx3," >",path,"strand/count/HTseq/",outs,
                   "-s-yes",".txt","\n")
  cmd_04 <- paste0("htseq-count -i gene_name -n 4 -s reverse -r pos ",
                   "-a 10 ",bams," ",inx3," >",path,"strand/count/HTseq/",outs,
                   "-s-reverse",".txt","\n")
  for (i in cmd_02) {cat(i, file = shell, append = T)}
  for (i in cmd_03) {cat(i, file = shell, append = T)}
  ...
  print(paste0("nohup bash ",shell, " >",
               path,"Log/",basename(shell),".log"," 2>&1 &"))
  #用STAR做定量
  shell <- paste0(path,"src/run_shy_cnt_c.sh")
  cat("#!/bin/bash\n", file = shell)
  prex <- c("KO-1","KO-2","WT-1","WT-2")
  lay1 <- paste0(path,"trim/",prex,"_1_val_1.fq.gz")
  lay2 <- paste0(path,"trim/",prex,"_2_val_2.fq.gz")
  cmd_05 <- paste0("STAR --runThreadN 8 --readFilesCommand zcat ",
                   "--quantMode TranscriptomeSAM GeneCounts ",
                   "--outMultimapperOrder Random --outSAMtype BAM SortedByCoordinate ",
                   "--outFileNamePrefix ",path,"strand/bam_quant/",prex," ",
                   "--outSAMprimaryFlag AllBestScore --twopassMode Basic ",
                   "--genomeDir ",inx1," --readFilesIn ",lay1," ",lay2,"\n")
  for (i in cmd_05) {cat(i, file = shell, append = T)}
  print(paste0("nohup bash ",shell, " >",
               path,"Log/",basename(shell),".log"," 2>&1 &"))
  #用featureCounts做定量
  shell <- paste0(path,"src/run_shy_cnt_d.sh")
  cat("#!/bin/bash\n", file = shell)
  bams <- list.files(paste0(path,"strand/bam"), pattern = "bam$", full.names = T)
  safs <- "/Reference/aaSHY/Ensembl99/GRCm38/gtf/GRCm38_Ensembl_rmsk_TE.saf"
  cmd_06 <- paste0("#featureCounts -T 16 -g gene_name -O -M ",
                   "-s 0 ",
                   "-p -P -d 0 -D 600 -B -C ", 
                   "-a ", inx3, " -o ", path,"strand/count/fC/rrpp_oi-s0MpPdDBC-pos.txt ",
                   paste0(bams[c(2,4,6,8)],collapse = " "), "\n")
  cmd_07 <- paste0("#featureCounts -T 16 -g gene_name -O -M ",
                   "-s 0 ",
                   "-p -P -d 0 -D 600 -B -C ", 
                   "-a ", inx3, " -o ", path,"strand/count/fC/rrpp_oi-s0MpPdDBC-neg.txt ",
                   paste0(bams[c(1,3,5,7)],collapse = " "), "\n")
  cmd_08 <- paste0("featureCounts -T 16 -g GeneID -O -M ",
                   "-F 'SAF' -s 0 ",
                   "-p -P -d 0 -D 600 -B -C ", 
                   "-a ", safs, " -o ", path,"strand/count/fC/rrpp_oi-s0MpPdDBC-pos-te.txt ",
                   paste0(bams[c(2,4,6,8)],collapse = " "), "\n")
  cmd_09 <- paste0("featureCounts -T 16 -g GeneID -O -M ",
                   "-F 'SAF' -s 0 ",
                   "-p -P -d 0 -D 600 -B -C ", 
                   "-a ", safs, " -o ", path,"strand/count/fC/rrpp_oi-s0MpPdDBC-neg-te.txt ",
                   paste0(bams[c(1,3,5,7)],collapse = " "), "\n")
  for (i in cmd_06) {cat(i, file = shell, append = T)}
  for (i in cmd_07) {cat(i, file = shell, append = T)}
  for (i in cmd_08) {cat(i, file = shell, append = T)}
...
################################################################################
#03、DESeq2做差异分析 [by TEtranscripts]
{
  #准备文件, 制作基因的metadata信息
  gtf1 <- import(inx3)
  gtf2 <- as.data.frame(gtf1)
  gtf3 <- gtf2[which(gtf2$type %in% c("transcript")),c(10,12,5)]
  tef1 <- as.data.frame(import(inx4))
  tef2 <- tef1[,c(10,1:3,5,12,13)]
  tef3 <- data.frame(gene_name = paste0(tef2$gene_id,":",tef2$family_id,":",tef2$class_id), 
                     strand = "*")
  
  shy_diff <- function(mark,stra,Class,vsvs){
    geneFile = paste0(path, "strand/count/TEtranscripts/",mark,".cntTable")
    tmp1 <- read.table(geneFile, header = T, 
                       check.names = F, stringsAsFactors = F, row.names = 1)
    colnames(tmp1) <- sapply(str_split(basename(colnames(tmp1)),pattern = "\\."), "[", 1)
    dfclas <- data.frame(row.names = colnames(tmp1), Class, stringsAsFactors = T)
    geneAA <- tmp1[grep(rownames(tmp1), pattern = ":", invert = T),]
    ...
    write.csv(geneAA, paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.csv"))
    upup <- subset(geneAA, log2FoldChange > log2(1.5) & padj < 0.05)
    down <- subset(geneAA, log2FoldChange < -log2(1.5)& padj < 0.05)
    write.csv(upup,paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.upup.csv"))
    write.csv(down,paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.down.csv"))
    tmp2 <- as.data.frame(geneAA)[,c(2,5,6)]
    tmp2$gene_name <- rownames(tmp2)
    colnames(tmp2)[1:3] <- paste0(colnames(tmp2)[1:3]," (",ST,")")
    res1 <<- left_join(res1, tmp2, by = "gene_name")
    #
    #
    cpm1 <- tmp1
    cpm1 <- as.data.frame(apply(cpm1, 2, function(x){x/sum(x)*1000000}))
    cpm1$gene_name <- rownames(cpm1)
    colnames(cpm1)[1:4] <- paste0(colnames(cpm1)[1:4]," (",ST,")")
    res2 <<- left_join(res2, cpm1, by = "gene_name")
    #
    #
    #
    #
    #
    #
    tmp3 <- tmp1[grep(rownames(tmp1), pattern = ":", invert = F),]
    teteAA <- tmp3
    teteAA <- DESeq2::DESeqDataSetFromMatrix(countData = teteAA, 
                                             colData = dfclas, design = ~Class)
    teteAA <- teteAA[rowMeans(BiocGenerics::counts(teteAA)) > 5,]
    teteAA <- DESeq2::DESeq(teteAA,)
    teteAA <- DESeq2::results(teteAA, contrast = vsvs)
    write.csv(teteAA, paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.csv"))
    upup <- subset(teteAA, log2FoldChange > log2(1.5) & padj < 0.05)
    down <- subset(teteAA, log2FoldChange < -log2(1.5)& padj < 0.05)
    write.csv(upup,paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.upup.csv"))
    write.csv(down,paste0(path,"strand/DESeq2/TEtranscripts/",mark, ".geneOnly.down.csv"))
    tmp4 <- as.data.frame(teteAA)[,c(2,5,6)]
    tmp4$gene_name <- rownames(tmp4)
    colnames(tmp4)[1:3] <- paste0(colnames(tmp4)[1:3]," (",ST,")")
    res3 <<- left_join(res3, tmp4, by = "gene_name")
    #
    #
    cpm3 <- tmp3
    cpm3 <- as.data.frame(apply(cpm3, 2, function(x){x/sum(x)*1000000}))
    cpm3$gene_name <- rownames(cpm3)
    colnames(cpm3)[1:4] <- paste0(colnames(cpm3)[1:4]," (",ST,")")
    res4 <<- left_join(res4, cpm3, by = "gene_name")
  }
  res1 <- as.data.frame(distinct(gtf3))
  res2 <- as.data.frame(distinct(gtf3))
  res3 <- as.data.frame(distinct(tef3))
  res4 <- as.data.frame(distinct(tef3))
  for (i in seq(2)) {
    print(i)
    MK <- c("pos","neg")[i]
    ST <- c("+", "-")[i]
    shy_diff(mark = MK, stra = ST,
             Class = factor(c("KO","KO","WT","WT")), vsvs = c("Class","KO","WT"))
  }
  res1[is.na(res1)] <- "NA"
  res3[is.na(res3)] <- "NA"
  openxlsx::write.xlsx(x = list("DESeq2" = res1, "CPM" = res2), 
                       file = paste0(path,"strand/DESeq2/TEtranscripts/rrpp_oiss-byTEtranscripts-geneOnly.xlsx"))
  openxlsx::write.xlsx(x = list("DESeq2" = res3, "CPM" = res4), 
                       file = paste0(path,"strand/DESeq2/TEtranscripts/rrpp_oiss-byTEtranscripts-teOnly.xlsx"))
}
################################################################################
#04、DESeq2做差异分析 [by featureCounts]
{
  #准备文件, 制作基因的metadata信息
  gtf1 <- import(inx3)
  gtf2 <- as.data.frame(gtf1)
  ...
  shy_diff <- function(mark,stra,Class,vsvs){
    geneFile = paste0(path, "strand/count/fC/rrpp_oi-s0MpPdDBC-",mark,"-gene-2.txt")
    tmp1 <- read.table(geneFile, header = T, 
                       check.names = F, stringsAsFactors = F, row.names = 1)
    colnames(tmp1) <- sapply(str_split(basename(colnames(tmp1)),pattern = "\\."), "[", 1)
    dfclas <- data.frame(row.names = colnames(tmp1), Class, stringsAsFactors = T)
    ...
    geneAA <- DESeq2::DESeq(geneAA,)
    geneAA <- DESeq2::results(geneAA, contrast = vsvs)
    write.csv(geneAA, paste0(path,"strand/DESeq2/fC/",mark, ".geneOnly.csv"))
    upup <- subset(geneAA, log2FoldChange > log2(1.5) & padj < 0.05)
    down <- subset(geneAA, log2FoldChange < -log2(1.5)& padj < 0.05)
    write.csv(upup,paste0(path,"strand/DESeq2/fC/",mark, ".geneOnly.upup.csv"))
    write.csv(down,paste0(path,"strand/DESeq2/fC/",mark, ".geneOnly.down.csv"))
    tmp2 <- as.data.frame(geneAA)[,c(2,5,6)]
    tmp2$gene_name <- rownames(tmp2)
    colnames(tmp2)[1:3] <- paste0(colnames(tmp2)[1:3]," (",ST,")")
    res1 <<- left_join(res1, tmp2, by = "gene_name")
    ...
    #
    #
    teteFile = paste0(path, "strand/count/fC/rrpp_oi-s0MpPdDBC-",mark,"-te-2.txt")
    tmp3 <- read.table(teteFile, header = T, 
                       check.names = F, stringsAsFactors = F, row.names = 1)
    colnames(tmp3) <- sapply(str_split(basename(colnames(tmp3)),pattern = "\\."), "[", 1)
    nm_t <- data.frame(gene_id = rownames(tmp3), strand = "*")
    nm_t <- left_join(nm_t, distinct(tef2[,c(1,6,7)]), by="gene_id")
    nm_t$gene_name <- paste0(nm_t$gene_id,":",nm_t$family_id,":",nm_t$class_id)
    rownames(tmp3) <- nm_t$gene_name
    ...
    teteAA <- DESeq2::DESeq(teteAA,)
    teteAA <- DESeq2::results(teteAA, contrast = vsvs)
    write.csv(teteAA, paste0(path,"strand/DESeq2/fC/",mark, ".teOnly.csv"))
    upup <- subset(teteAA, log2FoldChange > log2(1.5) & padj < 0.05)
    down <- subset(teteAA, log2FoldChange < -log2(1.5)& padj < 0.05)
    write.csv(upup,paste0(path,"strand/DESeq2/fC/",mark, ".teOnly.upup.csv"))
    write.csv(down,paste0(path,"strand/DESeq2/fC/",mark, ".teOnly.down.csv"))
    tmp4 <- as.data.frame(teteAA)[,c(2,5,6)]
    tmp4$gene_name <- rownames(tmp4)
    colnames(tmp4)[1:3] <- paste0(colnames(tmp4)[1:3]," (",ST,")")
    res3 <<- left_join(res3, tmp4, by = "gene_name")
  ...
  res3[is.na(res3)] <- "NA"
  openxlsx::write.xlsx(x = list("DESeq2" = res1, "CPM" = res2), 
                       file = paste0(path,"strand/DESeq2/fC/rrpp_oiss-byfeatureCounts-geneOnly.xlsx"))
  openxlsx::write.xlsx(x = list("DESeq2" = res3, "CPM" = res4), 
                       file = paste0(path,"strand/DESeq2/fC/rrpp_oiss-byfeatureCounts-teOnly.xlsx"))
}
#
#
#
#
################################################################################
#05、stringtie组装转录组 + UCSC格式
{
  ### 链特异*8
  shell <- paste0(path,"src/run_shy_cnt_e.sh")
  cat("#!/bin/bash\n", file = shell)
  bams <- list.files(paste0(path,"strand/bam"), pattern = "bam$", full.names = T)
  prex <- gsub(".sort.bam","",basename(bams))
  cmd_10 <- paste0("stringtie ",bams," ",
                   "-p 20 --rf -u --viral -G ", inx3, " -o ", path,
                   "strand/stringtie/",prex,"-rf-u-viral.gtf","\n")
  for (i in cmd_10) {cat(i, file = shell, append = T)}
  print(paste0("nohup bash ",shell, " >",
               path,"Log/",basename(shell),".log"," 2>&1 &"))
  ### 不特异*4
  shell <- paste0(path,"src/run_shy_cnt_e.sh")
  cat("#!/bin/bash\n", file = shell)
  bams <- list.files(paste0(path,"bam"), pattern = "bam$", full.names = T)
  prex <- gsub(".bam","",basename(bams))
  cmd_10 <- paste0("stringtie ",bams," ",
                   "-p 20 --rf -u --viral -G ", inx3, " -o ", path,
                   "strand/stringtie/",prex,"-rf-u-viral.gtf","\n")
  for (i in cmd_10) {cat(i, file = shell, append = T)}
  ...
  #
  ### UCSC格式
  gtfs <- paste0(path,"strand/stringtie/",prex,"-rf-u-viral.gtf")
  for (i in seq_along(gtfs)) {
    print(i)
    red1 <- as.data.frame(import(gtfs[i]))
    red1 <- red1[which(red1$seqnames %in% c(seq(19),"X","Y")),]
    red1$seqnames <- paste0("chr",red1$seqnames)
    red2 <- makeGRangesFromDataFrame(df = red1, keep.extra.columns = T)
    export(object = red2, format = "gtf",
           con = paste0(path,"strand/stringtie/",prex[i],"-rf-u-viral-ucsc.gtf"))
  }
}
