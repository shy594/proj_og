###OE
#write by Sun Haiayng at 2025.02.06
#01、搭环境
{
  rm(list=ls())
...
#02、给索引
{
  path = "/ChIP_seq_2/aaSHY/ghyuu/rnaSEQ/thejjuuzz/"
...
#03、跑命令
{
  ...
  cmd_10 <- paste0("cat ", paste0(path,"stringtie/jjuuzz.merge.gtf "), 
...
#09、MA plot TE sites
{
  shy_repSite <- function(resdFile, saveFile){
    ...
    gfcc <- gf00[grep(rownames(gf00), pattern = ".*MM-int.*|.*MT2_Mm.*", invert = T),]
    #开始作图
    p_1 <- ggplot() +
      theme_bw() +
      ...
      scale_color_manual(breaks = c("MM-int","MT2_Mm","Others"),values = c("red","purple","gray")) +
  ...
  shy_repSite(resdFile = paste0(path,"DESeq2/jjuuzz.te.site.teOnly.csv"),
              saveFile = paste0(path,"PLOT/jjuuzz.te.site.teOnly.MM.pdf"))
}
################################################################################
#10、PCA
{
...
{
  prex <- c("OE-1","OE-2","WT-1","WT-2")
  MMss <- openxlsx::read.xlsx(paste0(path,"countUniq/jjuuzz.cpm.onlyTE.site.xlsx"))
  colnames(MMss) <- c(prex,"theName")
  gggg <- data.frame(TE = sapply(str_split(MMss$theName,":"),"[",2),
                     family = sapply(str_split(MMss$theName,":"),"[",3),
                     class = sapply(str_split(MMss$theName,":"),"[",4))
  ...
  shy_d <- function(grou="jjuuzz",mark) {
    MM <- MMss
    MM <- MM[grep(mark,sapply(str_split(MM$theName,":"),"[",2)),]
    MM <- MM[rowSums(MM[,1:4]==0) < 4,]
    MM <- tidyr::pivot_longer(data = MM, cols = -theName, names_to = "group", values_to = "cpms")
    vaue <- wilcox.test(x = MM$cpms[grep("OE",MM$group)],
                        y = MM$cpms[grep("WT",MM$group)], alternative = "greater")
    print(vaue$p.value)
    #
    MM$marks <- NA
    MM$marks[grep("OE",MM$group)] <- "Overexpression"
    MM$marks[grep("WT",MM$group)] <- "WildType"
    MM$marks <- factor(MM$marks, levels = c("WildType", "Overexpression"))
