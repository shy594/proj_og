#write by Sun Haiayng at 2024.08.09
#01、搭环境
{
  rm(list=ls())
...
#02、给索引
{
  path = "/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/"
...
#03、跑命令
{
  ...
  cmd_01 <- paste0("trim_galore --phred33 --fastqc -j 2 --clip_R1 10 --clip_R2 10 ",
                   "-o ", path, "trim --paired ",lay1," ",lay2,"\n")
  lay1 <- paste0(path,"trim/",prex,"_1_val_1.fq.gz")
  lay2 <- paste0(path,"trim/",prex,"_2_val_2.fq.gz")
  cmd_03 <- paste0("STAR --runThreadN 4 --readFilesCommand zcat ",
  ...
  cmd_16 <- paste0("#TEtranscripts -t ",
                   ...
                   " --GTF ",inx3," --TE ",inx4," --sortByPos --mode multi ",
                   "--project ghyvv --outdir ",paste0(path,"dumpROOM"),"\n")
...
{
  #tmp1 <- "/ChIP_seq_2/aaSHY/Prmt1/RNAseq/202404/ghyvvOE/count/ghyvv_sigdiff_gene_TE.txt"
  #tmp1 <- read.table(tmp1)
  #tmp2 <- "/ChIP_seq_2/aaSHY/Prmt1/RNAseq/202404/ghyvvOE/dumpROOM/ghyvv_sigdiff_gene_TE.txt"
...
#04、DESeq2：Gene、TE、TE sites
{
  ...
  shy_diff(mark = "ghyvv-",
           geneFile = paste0(path, "count/ghyvv.cntTable"),
           siteFile = paste0(path, "count/ghyvv.site.cntTable"),
           Class = factor(c("DD","DD","DD","WT","WT","WT")), VS = c("Class","DD","WT"))
}
################################################################################
#05、CPM Excel
{
  cnt1 <- read.table("/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/count/ghyvv.cntTable", header = T, sep = "\t", row.names = 1)
  colnames(cnt1) <- c("ghyvv-1","ghyvv-2","ghyvv-3","ctrl-1","ctrl-2","ctrl-3")
  ...
  openxlsx::write.xlsx(x = tmp1, file = "/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/count/ghyvv-.cpm.onlyTE.xlsx")
  openxlsx::write.xlsx(x = tmp2, file = "/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/count/ghyvv-.cpm.onlyGene.xlsx")
  openxlsx::write.xlsx(x = tmp3, file = "/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/count/ghyvv-.cpm.allGeneTE.xlsx")
}
################################################################################
...
#08、MA
{
  shy_teMA <- function(difffile, savefile){
    ...
    just <- length(which(rf01$pvalue<0.05 & abs(rf01$log2FoldChange)>log2(1.5)))
    if (just != 0) {
      rf01$colo[-(which(rf01$pvalue<0.05 & abs(rf01$log2FoldChange)>log2(1.5)))] <- "other"
    } else {
      rf01$colo <- "other"
    }
    rf02 <- rf01
    ...
    p_01 <- ggplot() +
      ...
      scale_color_manual(values = c(ggsci::pal_igv()(51)[1:c(length(unique(rf02$colo))-1)],"grey80")) +
  ...
  shy_teMA(difffile = paste0(path,"DESeq2/ghyvv-.te.GeneBackGround.csv"),
           savefile = paste0(path,"PLOT/ghyvv-.te.GeneBackGround.MA-v2.pdf"))
  shy_teMA(difffile = paste0(path,"DESeq2/ghyvv-.teOnly.csv"),
           savefile = paste0(path,"PLOT/ghyvv-.teOnly.MA-v2.pdf"))
}
#######################################################################
#09、MA plot TE sites
{
...
{
  #----------------------------------PCA-batch----------------------------------
  cnt1 <- read.table(paste0(path, "count/ghyvv.cntTable"),
...
#11、pheatmap
{
  ...
  shy_pheatmap(mark = "heat", readsFile = paste0(path, "count/ghyvv.cntTable"),
...
#12、Up TEs loci number [TElocal]
{
  shy_lociNum <- function(resdFile, saveFile){
    ...
    p_1 <- ggplot(forP, aes(Var1, Freq, fill = clas)) + 
      ...
      ylab("Number of Loci") + 
      xlab(NULL) +
      labs(title = expression(deRegulated~TEs~"in"~italic(ghyvv)^{"+/+"}~ESCs)) +
  ...
  shy_lociNum(resdFile = paste0(path, "DESeq2/ghyvv-.te.site.teOnly.upup.csv"),
              saveFile = paste0(path, "PLOT/TElociNum/ghyvv-.lociNum.teOnly.upup.pdf"))
  shy_lociNum(resdFile = paste0(path, "DESeq2/ghyvv-.te.site.teOnly.down.csv"),
  ...
  shy_fusion_stringTie <- function(countFile, Class, gtfFile, whichTE, contrast){
    ...
    p_02 <- ggplot() +
      ...
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
            panel.border = element_rect(colour = "black", linewidth = 0.8),
            axis.line = element_line(), axis.text = element_text(family = "serif")) +
      xlab(label = expression(Log2~(Fold~Change~of~italic(gene)^{"*/*"}~ESCs))) +
    ...
    ggsave(plot = p_02, 
           ...
           filename = paste0(path, "PLOT/chimeric/ghyvv-.chimeric.",whichTE,".pdf"))
  }
  shy_fusion_stringTie(countFile = paste0(path, "stringtie/count/ghyvv.cntTable"), 
                       Class = c("DD","DD","DD","WT","WT","WT"),
                       contrast = c("Class","DD","WT"),
                       gtfFile = paste0(path, "stringtie/ghyvv.merge.R.gtf"),
                       whichTE = "MT2_Mm")#MT2_Mm#MERVL-int)
}
#######################################################################
#14、enrich KEGG
{
  resFile <- "/ChIP_seq_2/aaSHY/ghyuu/publicData/ghyvvInhibit/DESeq2/ghyvv-.geneOnly.csv"
  resdata <- as.data.frame(read.csv(resFile, header = T, stringsAsFactors = F))
  ...
  ggsave(plot = plot,
         ...
         filename = paste0(path,"PLOT/enrich/ghyvv-.KEGG.pdf"))
}
#######################################################################
...
#16、GSEA
{
  shy_gsea <- function(resFile, savePath){
    ...
    pdf(file = paste0(savePath, 
                      str_split(basename(resFile),".csv")[[1]][1], ".GSEA.pdf"), width = 12, height = 10)
    print(p_a)
    print(p_1)
    print(p_2)
    print(p_3)
  ...
  shy_gsea(resFile = paste0(path, "DESeq2/ghyvv-.geneOnly.csv"), 
           savePath = paste0(path, "PLOT/enrichGSEA/"))
}
#######################################################################
#17、python GSEA
{
  temp <- read.csv(paste0(path, "DESeq2/ghyvv-.geneOnly.csv"), header = T)
  temp <- temp[, c("X", "log2FoldChange")]
  temp <- temp[order(temp$log2FoldChange, decreasing = T),]
  write.table(x = temp, file = paste0(path, "DESeq2/ghyvv.res.rnk"), quote = F, sep = "\t", col.names = F, row.names = F)
  cmd_12 <- paste0("gseapy prerank -g ","/Reference/aaSHY/GSEAgmt/TwoGenes.gmt -r ",
                   paste0(path, "DESeq2/ghyvv.res.rnk"), " -p 10 -o ", 
                   paste0(path, "PLOT/enrichGSEA/"))#conda activate base gseapy 0.10.8
}
#######################################################################
#18、UCSC Track
#https://genome.ucsc.edu/s/hysun088/ghyvvOE
{
...
#保留了参数: #"bamCompare -p 10 --scaleFactorsMethod SES --sampleLength 100000 "
{
  ...
  cmd_01 <- paste0("multiBigwigSummary bins ", 
                   ...
                   " --smartLabels",
                   " -o ",paste0(path,"PLOT/Correlation/ghyvv.cor.npz","\n"))
  cmd_02 <- paste0("plotCorrelation -in ",path,
                   "PLOT/Correlation/ghyvv.cor.npz ",
                   "-c pearson -p heatmap --plotFileFormat pdf --removeOutliers ",
                   "--colorMap bwr -o ", path,"PLOT/Correlation/ghyvv.cor.pdf ",
                   "--outFileCorMatrix ",path,"PLOT/Correlation/ghyvv.cor.index.txt","\n")
...
#20、哪些是滋养层基因
{
  ...
  #吕老师说有些不是滋养层基因
  #
  data <- read.table(paste0(path,"count/ghyvv.cntTable"), 
...
#######################################################################
#21、对ghyvv OE后激活的2细胞基因排个序
{
  gata <- read.csv(file = paste0(path,"DESeq2/ghyvv-.geneOnly.csv"),
  ...
  openxlsx::write.xlsx(x = wr01, 
                       file = paste0(path,"DESeq2/ghyvv-.geneOnly.2C.gene.xlsx"), asTable = T)
}
#######################################################################
#22、可变剪接
{
